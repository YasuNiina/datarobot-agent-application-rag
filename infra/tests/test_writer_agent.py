...
def pulumi_mocks(monkeypatch):
    ...
    # Mock out the FastApi and just expose the runtime credentials as it is the only public
    # interface required for this module.
    mock_fastapi_module = MagicMock()
    mock_fastapi_module.custom_model_credential_runtime_parameters = []
    monkeypatch.setitem(sys.modules, "infra.web", mock_fastapi_module)
